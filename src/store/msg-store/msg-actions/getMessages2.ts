...
export const getMessages2 = async (self: MsgStore) => {
  ...
  if (hasRealmData.msg) {
    ...
    if (rs.totalMessages === 0) {
      display({
        name: 'getMessages2',
        preview: 'No messages in Realm. RESTORING...',
        important: true,
        value: { rs },
      })
    ...
    display({
      name: 'getMessages2',
      preview: `NO msg object in Realm - Restoring all`, // yeah but does this work if theres a msg object but empty of msgs
      important: true,
      value: { hasRealmData },
    })
  ...
  // const newRoute = 'messages?limit=2000'
